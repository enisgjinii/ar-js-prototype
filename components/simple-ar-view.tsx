...
export default function SimpleARView({ onBack }: SimpleARViewProps) {
  ...
  const startSimpleAR = async () => {
    ...
    try {
      ...
      if (!supported) {
        throw new Error(
          'AR not supported on this device. Make sure you have ARCore installed.'
        );
      }
      ...
      for (const profile of sessionProfiles) {
        try {
          ...
          console.warn(
            `⚠️ Failed to start AR session with profile "${profile.label}"`,
            sessionError
          );
        }
      }
      ...
      if (!session || !sessionProfileUsed) {
        throw new Error(
          'Unable to start an AR session with the available WebXR features on this device.'
        );
      }
      ...
      for (const spaceType of preferredSpaces) {
        try {
          ...
          console.warn(
            `ℹ️ Reference space "${spaceType}" not supported on this device.`,
            spaceError
          );
        }
      }
      ...
      if (!referenceSpace || !activeReferenceSpaceType) {
        throw new Error(
          'This device does not expose a compatible WebXR reference space. Try Chrome on Android with ARCore.'
        );
      }
      ...
      const testMaterial = new (BABYLON as any).StandardMaterial(
        'testMat',
        scene
      );
      ...
      const handleTap = () => {
        ...
        const material = new (BABYLON as any).StandardMaterial(
          `mat-${tapCount}`,
          scene
        );
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20">
          <div className="text-center">
            ...
            <p className="text-white text-sm mt-2">
              Simple AR - Just tap to place objects!
            </p>
          </div>
        </div>
      )}
