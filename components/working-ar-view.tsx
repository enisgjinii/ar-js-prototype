...
export default function WorkingARView({ onBack }: WorkingARViewProps) {
  ...
  const [referenceSpaceType, setReferenceSpaceType] = useState<string | null>(
    null
  );
  ...
  const startAR = async () => {
    ...
    try {
      if (!(navigator as any).xr) {
        throw new Error(
          'WebXR not supported. Use Chrome on Android with ARCore.'
        );
      }
      ...
      if (!supported) {
        throw new Error(
          'AR not supported on this device. Try Chrome on Android.'
        );
      }
      ...
      new (BABYLON as any).HemisphericLight(
        'light',
      ...
      try {
        ...
        console.warn(
          'WebXRDefaultExperience.CreateAsync failed, falling back to lower-level session handling',
          e
        );
      }
      ...
      if (!session) {
        ...
        const spaceCandidates: XRReferenceSpaceType[] = [
          'local-floor',
          'bounded-floor',
          'local',
          'viewer',
        ...
        for (const s of spaceCandidates) {
          try {
            referenceSpace = await (session as XRSession).requestReferenceSpace(
              s
            );
        ...
        if (!referenceSpace) {
          throw new Error(
            'No compatible reference space available on this device.'
          );
        }
      }
      ...
      const testCube = (BABYLON as any).MeshBuilder.CreateBox(
        'testCube',
        { size: 0.12 },
        scene
      );
      ...
      const reticle = (BABYLON as any).MeshBuilder.CreateTorus(
        'reticle',
        { diameter: 0.15, thickness: 0.01, tessellation: 24 },
        scene
      );
      ...
      if (xr && xr.baseExperience && xr.baseExperience.featuresManager) {
        try {
          ...
          const hitTestFeature = featuresManager.enableFeature(
            (BABYLON as any).WebXRHitTest.Name,
            'latest'
          );
      ...
      const handleTap = () => {
        tapCount++;
        const sphere = (BABYLON as any).MeshBuilder.CreateSphere(
          `sphere-${tapCount}`,
          { diameter: 0.08 },
          scene
        );
        ...
        const mat = new (BABYLON as any).StandardMaterial(
          `mat-${tapCount}`,
          scene
        );
        mat.diffuseColor = new (BABYLON as any).Color3(
          Math.random(),
          Math.random(),
          Math.random()
        );
          ...
          {showDebugPanel && (
            <div className="absolute top-4 left-4 z-30 bg-black/60 text-white p-3 rounded-md text-xs max-w-xs">
              ...
              <div className="space-y-1">
                <div>
                  XR mode: <span className="font-medium ml-1">{xrMode}</span>
                </div>
                <div>
                  Hit-test:{' '}
                  <span className="font-medium ml-1">
                    {hitTestEnabled ? 'enabled' : 'disabled'}
                  </span>
                </div>
                <div>
                  Reference space:{' '}
                ...
                <div>
                  Objects:{' '}
                  <span className="font-medium ml-1">{objectsPlaced}</span>
                </div>
              </div>
