...
export default function AFrameARView({ onBack }: AFrameARViewProps) {
  ...
  const startAFrameAR = async () => {
    ...
    try {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        throw new Error(
          'getUserMedia not supported in this browser. Use a modern mobile browser over HTTPS.'
        );
      }
      ...
      if (!(window as any).AFRAME) {
        ...
        setError(
          'A-Frame failed to load. Check network or CSP blocking external scripts.'
        );
        setIsLoading(false);
        return;
      }
      ...
      const handleTap = (event: MouseEvent) => {
        ...
        box.setAttribute(
          'animation__bob',
          `property: position; dir: alternate; dur: 800; easing: easeInOutSine; loop: true; to: ${x} ${y + 0.05} ${z}`
        );
  ...
  const cameraStatus = () => {
    ...
    return {
      ...
      permissionsApi: !!(
        navigator.permissions && (navigator as any).permissions.query
      ),
    };
  };
    ...
    <div className="w-full h-screen relative bg-black">
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20">
          <div className="text-center">
            ...
            <p className="text-white text-sm mt-2">
              Most reliable mobile AR solution
            </p>
      ...
      {!isARActive && (
        <div className="absolute top-4 right-4 z-10 bg-black/60 text-white px-3 py-2 rounded-md text-xs">
          ...
          <div>
            Permissions API: {cameraStatus().permissionsApi ? 'Yes' : 'No'}
          </div>
        </div>
      )}
