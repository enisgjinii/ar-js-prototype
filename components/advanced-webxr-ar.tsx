...
export default function AdvancedWebXRAR({ onBack }: AdvancedWebXRARProps) {
  ...
  const startAdvancedAR = async () => {
    ...
    try {
      ...
      const supportedSpaces: XRReferenceSpaceType[] = [
        'local-floor',
        'local',
        'viewer',
      ];
      ...
      if (!referenceSpace) {
        throw new Error(
          'No supported reference space found. Device may not support WebXR AR.'
        );
      }
        ...
        <div className="absolute inset-0 flex items-center justify-center z-20">
          <div className="text-center">
            ...
            <p className="text-white text-sm mt-2">
              Hit-test + Plane Detection
            </p>
            <p className="text-white text-xs mt-1 opacity-70">
              Objects placed on real surfaces!
            </p>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-20">
          <div className="text-center text-white">
            ...
            <p className="text-sm opacity-70">
              Initializing WebXR with hit-test
            </p>
          </div>
        </div>
      )}
      ...
      {isARActive && (
        <>
          ...
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10">
            <div
              className={`${surfaceDetected ? 'bg-green-600/80' : 'bg-yellow-600/80'} text-white px-3 py-1 rounded-full text-sm flex items-center gap-1`}
            >
              {surfaceDetected ? (
                <>
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-10">
            <div className="bg-black/70 text-white px-6 py-3 rounded-lg text-center max-w-sm">
              ...
              <p className="text-xs opacity-60">
                {surfaceDetected
                  ? 'Green ring shows placement location'
                  : 'Point at floors, tables, walls'}
              </p>
            </div>
          </div>
        </>
